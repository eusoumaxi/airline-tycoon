/**
 * @fileoverview Console reports: before/after metrics, oversupply, plan sample.
 */
import {
  CARGO_OVERSHOOT_CAP,
  DAY_SECONDS,
...
import { capacityOf, hubCodes, roundTripDuration } from "./model.ts";
import {
  type Aircraft,
  type AircraftPlan,
  CABIN_CLASSES,
...
/** Coverage, utilisation, and oversupply for a current or proposed plan. */
export interface Metrics {
  /** Demand coverage per class: served / demand (served is per-day capped). */
  coverage: Record<
    CabinClass,
    { offered: number; demand: number; served: number }
  >;
  ...
  perLine: Map<
    number,
    {
      offered: Record<CabinClass, number>;
...
/**
 * Coverage, utilisation, and oversupply for a set of flights.
 *
 * Offer is counted **per departure day**. Weekly totals can look healthy while one
 * day is oversupplied; the game regenerates demand each day.
 */
export function computeMetrics(
  ...
  for (const { aircraftId, flights } of flightsByAircraft) {
    ...
    if (flights.length > 0) {
      utils.push(Math.min(1, used / WEEK_SECONDS));
    }
  }
  ...
  const utilMax = utils.at(-1) ?? 0;
  const near100 = utils.filter((u) => u >= 0.99).length;

  const coverage: Metrics["coverage"] = {
    bus: { demand: 0, offered: 0, served: 0 },
    cargo: { demand: 0, offered: 0, served: 0 },
    eco: { demand: 0, offered: 0, served: 0 },
    first: { demand: 0, offered: 0, served: 0 },
  };
  ...
  for (const [lineId, line] of lines) {
    ...
    if (days) {
      perLine.set(lineId, { offered, over, served });
    }
  }
...
export function printRouteAnalysis(lines: Map<number, Line>): void {
  console.log(
    `\n${hr("═")}\n  ROUTE ANALYSIS (${lines.size} routes)  ·  demand x${DAYS_PER_WEEK} days\n${hr("═")}`
  );
  ...
  if (arr.length > 20) {
    console.log(`  ... and ${arr.length - 20} more routes`);
  }
}
...
export function printFleetAnalysis(aircraft: Aircraft[]): void {
  ...
  for (const [model, list] of byModel) {
    const s = list[0];
    const avgUtil =
      list.reduce((x, a) => x + a.utilizationPercentage, 0) / list.length;
    console.log(
...
export function printHubSummary(
  aircraft: Aircraft[],
  lines: Map<number, Line>
): void {
  console.log(`\n${hr("═")}\n  PER-HUB SUMMARY (proposed)\n${hr("═")}`);
  const hubCode = hubCodes(lines.values());
  ...
  for (const l of lines.values()) {
    ...
    for (let d = 0; d < DAYS_PER_WEEK; d++) {
      g.over += l.over[d].eco + l.over[d].bus + l.over[d].first;
    }
  }
  ...
  console.log(`  ${hr("─").slice(2)}`);
  for (const [hubId, g] of [...byHub.entries()].sort(
    (a, b) => b[1].planes - a[1].planes
  )) {
    console.log(
...
export function printComparison(before: Metrics, after: Metrics): void {
  ...
  const row = (label: string, b: string, a: string) =>
    console.log(
      `  ${label.padEnd(26)} ${b.padStart(16)}   ->  ${a.padStart(16)}`
    );
  ...
  row(
    "Aircraft >= 99% util",
    `${before.near100}/${before.flying}`,
    `${after.near100}/${after.flying}`
  );
  row("Per-day pax OVERSUPPLY", fmt(before.oversupply), fmt(after.oversupply));
  row(
    "Served value / week",
    money(before.servedValue),
    money(after.servedValue)
  );
  console.log(`  ${hr("─").slice(2)}`);
  ...
  console.log(`  ${hr("─").slice(2)}`);
  console.log(
    `  Δ Served value: ${delta >= 0 ? "+" : ""}${money(delta)} / week`
  );
}
 ...
 */
export function printOversupply(
  after: Metrics,
  lines: Map<number, Line>,
  n = 12
): void {
  ...
  const free = [...rows]
    .map((r) => ({
      ecoFree: r.l.weeklyDemand.eco - r.served.eco,
      name: r.l.name,
    }))
  ...
  console.log(`  ${hr("─").slice(2)}`);
  console.log(
    `  Uncovered ECO demand: ${fmt(freeTotal)} seats/week across ${free.length} routes (room for more aircraft).`
  );
}
...
function dailyOversupplyRows(
  ...
  for (const { aircraftId, flights } of flightsByAircraft) {
    ...
    for (const f of flights) {
      if (!lines.has(f.lineId)) {
        continue;
      }
      const day = Math.floor((f.takeOffTime % WEEK_SECONDS) / DAY_SECONDS);
      let days = offered.get(f.lineId);
      if (!days) {
        days = new Array(DAYS_PER_WEEK).fill(0);
        offered.set(f.lineId, days);
      }
      days[day] += perTrip;
    }
  }
  ...
  for (const [lineId, days] of offered) {
    ...
    for (let d = 0; d < DAYS_PER_WEEK; d++) {
      ...
      if (over > 0) {
        rows.push({ day: d, name: line.name, over });
      }
    }
  }
...
/**
 * Print per-day eco (and cargo) overshoot for the proposed plan.
 *
 * Economy overshoot above `ECO_OVERSHOOT_CAP` is a hard optimizer rule.
 * Cargo overshoot on passenger flights is informational (belly cargo is not gated).
 */
export function printDailyOvershoot(
  ...
  if (eco.length === 0) {
    console.log(
      "  (no route is eco-oversupplied on any day — every eco flight serves real demand)"
    );
  } else {
    console.log(
      `  Worst route-days (game shows this as the red negative "remaining"):`
    );
    for (const r of eco.slice(0, n)) {
      console.log(
        `    ${r.name.padEnd(13)} ${DAY_NAMES[r.day]}  −${fmt(r.over).padStart(6)} eco${r.over > ecoCap ? "   ✗ EXCEEDS CAP" : ""}`
      );
    }
  }
  console.log(`  ${hr("─").slice(2)}`);
  ...
  if (cargo.length === 0) {
    console.log(
      `  Cargo: no route oversupplied (freighter guard −${fmt(cargoCap)} t).`
    );
  } else {
    const worst = cargo[0];
    console.log(
...
export function printPlanSample(
  plans: AircraftPlan[],
  fleet: FleetIndex,
  lines: Map<number, Line>,
  n = 12
