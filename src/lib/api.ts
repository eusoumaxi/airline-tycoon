/**
 * @fileoverview HTTP client for Airlines Manager.
 *
 * Discover hubs, download planning, push schedules, buy/sell routes.
 * All requests use `AM_COOKIE`. Never call write helpers unless the CLI passed `--apply`.
 */
...
/** Headers for a document navigation (HTML pages). */
...
/** Headers for XHR JSON (`/network/planning/load/…`). */
...
/**
 * List hubs on `/network/planning`.
 *
 * Each hub is `data-airportid="…" data-hubId="…"> LHR`.
 */
export async function discoverHubs(): Promise<Hub[]> {
  ...
  if (!res.ok) {
    throw new Error(`GET ${url} -> ${res.status} (cookie expired?)`);
  }
  ...
  for (let m = re.exec(html); m; m = re.exec(html)) {
    ...
    seen.add(loadId);
    hubs.push({ airportId: Number(m[1]), code: m[3], loadId });
  }
  return hubs;
}

/** Download one hub's planning JSON (`GET /network/planning/load/<loadId>`). */
export async function fetchPlanning(
  planningId: number
): Promise<PlanningPayload> {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `GET ${url} -> ${res.status} ${res.statusText}. ` +
        `The cookie probably expired; update AM_COOKIE in .env.\n${body.slice(0, 300)}`
    );
  }
  const data = (await res.json()) as Partial<PlanningPayload>;
  if (!(data.aircraftDataArray && data.lineDataArray)) {
    throw new Error(
      `Unexpected response from ${url}: missing aircraftDataArray/lineDataArray`
    );
  }
  return data as PlanningPayload;
}

/** Download and return several hubs' planning. */
export function fetchAllPlannings(
  planningIds: number[]
): Promise<PlanningPayload[]> {
...
/**
 * Push one aircraft's week (`POST /network/planning/0/ajax`).
 *
 * Empty `added` clears the schedule. HTTP 200 can still be a game rejection
 * (granularity, category); those throw.
 */
export async function updatePlanning(plan: AircraftPlan): Promise<unknown> {
  ...
  const planningData =
    plan.added.length > 0
      ? JSON.stringify({ added: plan.added, aircraftId: plan.aircraftId })
...
function parseBuyableRoutes(html: string, country: string): BuyableRoute[] {
  ...
  for (const b of blocks) {
    if (!b.startsWith('<div class="hubListBox"')) {
      continue;
    }
    ...
    const iata =
      ...
      code?.toUpperCase() ??
      "";
    const demandPct = Number(
      /barDemandFill"\s+style="width:\s*([\d.]+)%/.exec(b)?.[1] ?? 0
    );
    if (!code) {
      continue;
    }
...
export async function fetchBuyableRoutes(
  hubLoadId: number,
  country: string
): Promise<BuyableRoute[]> {
...
async function fetchBuyToken(
  ...
  const html = await res.text();
  const token =
    /name="form\[_token\]"\s+value="([^"]+)"/.exec(html)?.[1] ??
    /form%5B_token%5D=([^"&']+)/.exec(html)?.[1];
  if (!token) {
    throw new Error(
      `No form[_token] found on ${url} (route may already be owned, or layout changed)`
    );
  }
  return token;
}
...
export async function buyRoute(
  hubLoadId: number,
  destCode: string
): Promise<string> {
  ...
  if (
    ...
    throw new Error(
      `Purchase rejected: ${text.replace(/\s+/g, " ").slice(0, 200)}`
    );
  }
  return "ok";
}
...
export async function sellLine(lineId: number): Promise<string> {
  ...
  const getRes = await fetch(url, {
    headers: { ...listHeaders(), referer: ref },
  });
  if (!getRes.ok) {
    throw new Error(`GET sell -> ${getRes.status}`);
  }
  ...
  if (!res.ok) {
    throw new Error(`POST sell -> ${res.status} ${res.statusText}`);
  }
